...
def run_method(X, 
    ...
    if method == 'nt':
        ...
        if device == '':
            device = torch.device(f"cuda:{get_freer_gpu()}" if torch.cuda.is_available() else "cpu")
        logging.info("Running on: ",device)
        ...
        random_stability(seed)
        fitted = pc(data=X, alpha=test_alpha, indep_test=test_name, uc_rule=5, uc_priority=3, show_progress=False, verbose=False)
        # fitted.draw_pydot_graph()
        W_est = fitted.G.graph.T
        elapsed = fitted.PC_elapsed
        logging.info(f'Time taken for Majority-PC: {round(elapsed,2)}s')

    elif method == 'pc_con':
        random_stability(seed)
        fitted = pc(data=X, alpha=test_alpha, indep_test=test_name, uc_rule=4, uc_priority=3, show_progress=False, 
                    verbose=False)
        # fitted.draw_pydot_graph()
        W_est = fitted.G.graph.T
        elapsed = fitted.PC_elapsed
        logging.info(f'Time taken for Conservative-PC: {round(elapsed,2)}s')
        ...
        try:
            ...
            logging.info("CAM failed, returning None")
            W_est = None
        elapsed = time.time() - start
